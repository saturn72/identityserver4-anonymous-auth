...
using IdentityServer4.Anonymous.Logging;
using System.Text.Json;

namespace IdentityServer4.Anonymous.Validation
{
    public sealed class AnonymousAuthorizationRequestValidator : IAnonymousAuthorizationRequestValidator
    {
        #region fields
        private readonly AnonymousAuthorizationOptions _options;
        private readonly IResourceValidator _resourceValidator;
        private readonly ILogger<AnonymousAuthorizationRequestValidator> _logger;
        #endregion
        #region ctor
        public AnonymousAuthorizationRequestValidator(
            IOptions<AnonymousAuthorizationOptions> options,
            IResourceValidator resourceValidator,
            ILogger<AnonymousAuthorizationRequestValidator> logger)
        {
        ...
        {
            _logger.LogDebug("Start anonymous authorization request validation");

            var request = new ValidatedAnonymousAuthorizationRequest
            {
            ...
            _logger.LogDebug("{clientId} anonymous request validation success", request.Client.ClientId);
            return Valid(request);
        }

        private AuthorizationRequestValidationResult Valid(ValidatedAnonymousAuthorizationRequest request) => new(request);

        private AuthorizationRequestValidationResult Invalid(
            ValidatedAnonymousAuthorizationRequest request,
            ...
            ClientSecretValidationResult clientValidationResult)
        {
            ...
            //////////////////////////////////////////////////////////
            // check if client allows anonymous flow
            //////////////////////////////////////////////////////////
            if (!request.Client.AllowedGrantTypes.Contains(Constants.AnonymousGrantType))
            {
                LogError("Client not configured for anonymous flow", Constants.AnonymousGrantType, request);
            ...
            {
                LogDebug($"Invalid redirect_uri: {redirectUri}", Constants.AnonymousGrantType, request);
                return Invalid(request, OidcConstants.AuthorizeErrors.UnauthorizedClient);
            }
        ...
        private async Task<AuthorizationRequestValidationResult> ValidateScopeAsync(ValidatedAnonymousAuthorizationRequest request)
        {
            //////////////////////////////////////////////////////////
        ...
        private void LogDebug(string message, string detail, ValidatedAnonymousAuthorizationRequest request)
        {
            var requestDetails = new AnonymousAuthorizationRequestValidationLog(request);
            _logger.LogDebug(message + ": {detail}\n{requestDetails}", detail, requestDetails);
        }
        private void LogError(string message, ValidatedAnonymousAuthorizationRequest request)
        {
            var requestDetails = new AnonymousAuthorizationRequestValidationLog(request);
            _logger.LogError(message + "\n{requestDetails}", requestDetails);
        }
        private void LogError(string message, string detail, ValidatedAnonymousAuthorizationRequest request)
        {
            var requestDetails = new AnonymousAuthorizationRequestValidationLog(request);
